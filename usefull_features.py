...
def group_by(data, by, method='diff', crit=1e-6):
    tmp = data.copy()
    tmp.sort(order=by)
    funcs = {'diff': lambda x: np.diff(
        x), 'ratio': lambda x: (x/np.roll(x, 1))[1:]}
    ii = [0] + (np.where(funcs[method](tmp[by]) >= crit)
...
class ConsoleProgress(object):
    ...
    def redraw(self):
        if self.chekRedraw(self.current):
            print(progress(self.current, self.total, self.scaleLength,
                           self.text, self.symbol), end='')  # , flush=True)
        self.current += 1

    def finalize(self):
        print(progress(self.total, self.total,
                       self.scaleLength, self.text, self.symbol))
